    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between h-20">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link to="/booking" className="btn-gold hidden md:inline-flex text-xs">
            احجزي موعدك
          </Link>
        ...
        <div className="md:hidden bg-ebony-soft border-t border-gold/20 px-6 py-6 flex flex-col gap-5">
          {NAV_LINKS.map(link => (
            <button
              key={link.href}
              onClick={() => handleNavClick(link.href)}
              className="text-right text-cream/80 hover:text-gold font-body text-base tracking-wide transition-colors"
            >
              {link.label}
            </button>
          ))}
